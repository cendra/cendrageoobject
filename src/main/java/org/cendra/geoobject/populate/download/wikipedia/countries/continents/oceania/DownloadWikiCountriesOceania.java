...
public class DownloadWikiCountriesOceania extends Download {

	public static final String FILE_NAME = "division_politica_oceania.html";
	public static String FULL_PATH = "downloads/wikipedia/countries/continents/oceania";
	private String urlString = "https://es.wikipedia.org/wiki/Anexo:Divisi%C3%B3n_pol%C3%ADtica_de_Ocean%C3%ADa";

	public DownloadWikiCountriesOceania(HTMLDocumentUtil htmlDocumentUtil,
	...
	public File download() throws Exception {
		...
		File oceaniaFile = this.getFileFullPath(FULL_PATH, true);

		String path = oceaniaFile.getAbsolutePath() + File.separatorChar
				+ FILE_NAME;

		// -------------------------------------------------------------

		List<CountryWikipediaOceania> countries = parse(path, oceaniaFile);

		// -------------------------------------------------------------

		File json = new File(oceaniaFile.getAbsolutePath() + File.separatorChar
				+ FILE_NAME + ".json");
	...
	public List<CountryWikipediaOceania> parse(String path,
			java.io.File oceaniaFile) throws Exception {

		List<CountryWikipediaOceania> countries = new ArrayList<CountryWikipediaOceania>();
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			if (i > 0) {
				CountryWikipediaOceania country = getCountry(row, oceaniaFile);
	...
	private CountryWikipediaOceania getCountry(Element row,
			java.io.File oceaniaFile) throws Exception {
		...
		java.io.File countryFile = new java.io.File(
				oceaniaFile.getAbsolutePath() + java.io.File.separatorChar
						+ iso3.toLowerCase());
		countryFile.mkdirs();
		...
		CountryWikipediaOceania country = new CountryWikipediaOceania();
