...
public class DownloadWikiCountriesEurpoa extends Download {

	public static final String FILE_NAME = "division_politica_europa.html";
	public static String FULL_PATH = "downloads/wikipedia/countries/continents/europa";
	private String urlString = "https://es.wikipedia.org/wiki/Anexo:Divisi%C3%B3n_pol%C3%ADtica_de_Europa";

	public DownloadWikiCountriesEurpoa(HTMLDocumentUtil htmlDocumentUtil,
	...
	public File download() throws Exception {
		...
		File europaFile = this.getFileFullPath(FULL_PATH, true);

		String path = europaFile.getAbsolutePath() + File.separatorChar
				+ FILE_NAME;

		// -------------------------------------------------------------

		List<CountryWikipediaEuropa> countries = parse(path, europaFile);

		// -------------------------------------------------------------

		File json = new File(europaFile.getAbsolutePath() + File.separatorChar
				+ FILE_NAME + ".json");
	...
	public List<CountryWikipediaEuropa> parse(String path, File europaFile)
			throws Exception {

		List<CountryWikipediaEuropa> countries = new ArrayList<CountryWikipediaEuropa>();
		...
		Element e = this.getHtmlDocumentUtil()
				.getElementsByElementName(doc.detachRootElement(), "table")
				.get(3);
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			if (i > 0) {
				CountryWikipediaEuropa country = getCountry(row, europaFile);
	...
	private CountryWikipediaEuropa getCountry(Element row, File europaFile)
			throws Exception {
		...
		java.io.File countryFile = new java.io.File(
				europaFile.getAbsolutePath() + java.io.File.separatorChar
						+ iso3.toLowerCase());
		countryFile.mkdirs();
